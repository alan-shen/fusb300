...
{
	//pull down both CC
	fusb3000_i2c_write(fusb_i2c_data->client,0x02,0x03);
	fusb3000_i2c_write(fusb_i2c_data->client,0x0B,0x07);
	fusb3000_i2c_write(fusb_i2c_data->client,0x0B,0x031);	
}
	...
	case IDLE_STATE:
		if (interrupt_value & 0x04) // I_WAKE interrupt
		{
			if (fusb_i2c_data->port_type == PORT_DRP) //DRP port
			{
				...
				//read status 
				status_value = fusb3000_i2c_read(fusb_i2c_data->client,0x40);
				if (status_value&0x80) // VBUSOK==1
				{
					fusb3000_i2c_read(fusb_i2c_data->client,0x42); // clear interrupt
...
static int fusb3000_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    fusb3000_i2c_write(fusb->client,0x06,0x00);
